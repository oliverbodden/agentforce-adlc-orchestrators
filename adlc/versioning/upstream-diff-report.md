...
| Corporate artifact repo | `https://code.corp.indeed.com/telecom/it-adlc` | Not cloned during this phase | Planned canonical artifact repo |
...
- Salesforce CLI is not optional for Indeed developer execution even though the upstream installer treats it as optional. Skills rely on `sf agent`, `sf project`, `sf data`, `sf apex`, and `sf api` command surfaces.
...
Use the latest Salesforce upstream as the standard baseline, then layer local behavior as a separate Indeed overlay:
